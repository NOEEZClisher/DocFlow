...
현재 버전에서는 TXT와 Markdown 문서를 앱 내부에서 읽고, DOCX 문서를 Markdown으로 변환해 확인하거나 저장하는 기본 구조를 제공합니다.

## MVP 범위

현재 버전은 제출물 문서를 탐색하고 읽는 데스크톱 리더기입니다.
TXT/Markdown은 그대로 읽고, DOCX는 문단/제목/표 구조를 Markdown으로 변환해 앱 안에서 확인하거나 Markdown 파일로 저장하는 흐름에 초점을 둡니다.
...
- 하단 상태 로그 표시
- 폴더 스캔, 텍스트 인코딩, Markdown 렌더링, DOCX 변환, Markdown 저장에 대한 pytest 테스트
